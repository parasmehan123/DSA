...
int main() {
    ...
    for(int T = 1; T <= test; T++){
        ...
        int n;
        cin>>n;
        vector<vector<int>> charCounts(n, vector<int>(6, 0));
        string s;
        REP(i,0,n){
            cin>>s;
            for(char c: s){
            ...
            charCounts[i][5] = s.length();
        }
 
        int ans = 0;
 
        REP(i,0,5){
            int curAns = 0, a = 0, _a = 0;
            priority_queue<int, vector<int>, greater<int>> Q;
            REP(j,0,n){
                if(2*charCounts[j][i] > charCounts[j][5]){
                    curAns++;
                    a += charCounts[j][i];
                    _a += charCounts[j][5] - charCounts[j][i];
                }
                else
                    Q.push(charCounts[j][5] - 2*charCounts[j][i]);
            }
            int curSum = 0;
            while(!Q.empty()){
                int x = Q.top();
